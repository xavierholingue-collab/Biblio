  ...
  try {
    ...
    if (chemin === "/api/connexion" && req.method === "POST") {
      ...
      return json(rep, { ok: true }, 200, {
        "Set-Cookie": `session=${jeton}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${DUREE_SESSION / 1000}`,
      });
    }

    if (chemin === "/api/deconnexion" && req.method === "POST") {
      return json(rep, { ok: true }, 200, { "Set-Cookie": "session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0" });
    }
